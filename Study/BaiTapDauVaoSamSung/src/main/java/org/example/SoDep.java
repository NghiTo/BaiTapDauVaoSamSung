...
public class SoDep
{
    public static void main(String[] args)
    {
        ...
        for (int i = 1; i <= testCase; i++)
        {
            ...
            int count = demSoDepInRange(x, y, soDep, m);
            System.out.println("#" + i + " " + count);
        }
    }

    public static int demSoDepInRange(int start, int end, int[] soDep, int soLanXuatHien)
    {
        int count = 0;
        ...
        for (int k = start; k <= end; k++)
        {
            ...
            if (totalSpecificDigits >= soLanXuatHien)
            {
                count++;
            }
        }
